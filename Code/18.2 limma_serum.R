...
mSet<-Read.TextDataTs(mSet, "data_normalized_no_label for metaboanalyst.serum.csv", "colmf");
...
df.covariate.BMI <- read.csv("covariate_result.serum.BMI.all.csv") %>%
  rename(p.BMI = P.Value)
df.covariate <- read.csv("covariate_result.serum.all.csv") %>%
  rename(p = P.Value)
...
df.covariate.BMI.sig <- read.csv("covariate_result.serum.BMI.csv") %>%
  rename(p.BMI = P.Value)
...
write.csv(df.covariate.BMI_w.anno, "df.covariate.BMI_w.anno.serum.csv", row.names = F)
...
saveRDS(df_metab_serum_Case_Ctrl.all%>% filter(MS2superclass != "Unknown") %>% dplyr::select(MS2superclass), 
        "serum_MS2superclass.rds")
df_metab_serum_Case_Ctrl.all.limma.sig <- df_metab_serum_Case_Ctrl.all %>%
...
df_metab_serum.limma.sig <- df.ttest.serum.all %>%
  dplyr::right_join(df_metab_serum_Case_Ctrl.all.limma.sig, by = "ID") %>%
  dplyr::select(ID, MZ, RT ,p.value, t.score) %>% 
  mutate(mode = ifelse(substr(ID, 1, 3) == "pos", "positive", "negative")) %>%
  tibble::column_to_rownames("ID") %>%
  rename(`m.z` = MZ, `r.t` = RT) %>%
  arrange(mode)
write.csv(df_metab_serum.limma.sig, "df_metab_serum.limma.sig_for_func.csv", row.names = F)
...
mSet_fun<-Read.PeakListData(mSet_fun, "df_metab_serum.limma.sig_for_func.csv");
