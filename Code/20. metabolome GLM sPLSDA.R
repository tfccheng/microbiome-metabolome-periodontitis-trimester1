...
library(hmdbQuery)
df.pld.splsda.metab.feces.MS2[df.pld.splsda.metab.feces.MS2$MS2Metabolite == "LysoPC 16:0", "MS2hmdb"] <- "HMDB0010382"
df.pld.splsda.metab.serum.MS2[df.pld.splsda.metab.serum.MS2$MS2Metabolite == "LysoPC 20:3", "MS2hmdb"] <- "HMDB0010393;HMDB0010394"



db_saliva <- df.pld.splsda.metab.saliva.MS2$MS2hmd %>%
  na_if("NA")
db_serum <- df.pld.splsda.metab.serum.MS2$MS2hmd %>%
  na_if("NA")
db_feces <- df.pld.splsda.metab.feces.MS2$MS2hmd %>%
  na_if("NA")

db_saliva <- db_saliva[!is.na(db_saliva)]
db_serum <- db_serum[!is.na(db_serum)]
db_feces <- db_feces[!is.na(db_feces)]
db_serum <- unlist(strsplit(db_serum, ";"))
db_feces <- unlist(strsplit(db_feces, ";"))
db <- rbind(data.frame(hmdb_id = db_saliva, sample = rep("Saliva", length(db_saliva))),
            data.frame(hmdb_id = db_serum, sample = rep("Serum", length(db_serum))),
            data.frame(hmdb_id = db_feces, sample = rep("Feces", length(db_feces))))


db$hmdb_id[10] <- "HMDB0011474"
db$hmdb_id[16] <- "HMDB0000187"
db$hmdb_id[38] <- "HMDB0004160"
db$hmdb_id[61] <- "HMDB0010388"
db$hmdb_id[75] <- "HMDB0010384"
db$hmdb_id[76] <- "HMDB0011128"
...
for (i in 1:dim(db)[1]){
  lk = HmdbEntry(prefix = "http://www.hmdb.ca/metabolites/",
                 id = db$hmdb_id[i])
  db$metabolite[i] <- lk@metabolite
  db$biospecimens[i] <- paste(lk@biospecimens, collapse = ";")
  db$disease_asso[i] <- paste(lk@diseases[1,], collapse = ";")
}

write.csv(db, 'metabolome sPLSDA/db_hmdb_saliva_serum_feces.csv', row.names = F)
